...
class AssetTrader(object):
    ...
    def __init__(self, asset: str, api_secret: str, api_key: str, passphrase: str, joblib_file: str, use_sandbox: bool = True):
        self.asset = asset
    ...
    def get_asset_details_last_hour(self, start: str, end: str, granularity: int = 3600):
        ...
        historic_data = pd.DataFrame(historic_data, columns=['timestamp', 'low', 'high', 'open', 'close', 'volume'])
    ...
    def predict(self, open_: float, high_: float, low_: float, close_: float, eth_volume_: float):
