...
def score_metrics(
    actual_values: np.array, model_predictions: np.array, mode: str
) -> dict:
    """
...
def strategy_simulation(
    ...
    for step, (_, series) in enumerate(test_dataset.iterrows()):
        ...
        if total_money <= 0:
            ...
            if action == "sell":
                ...
                assert (
                    asset_wallet_balance >= 0
                ), f"asset_wallet_balance is negative: {asset_wallet_balance}"
    ...
    percentage_gain_lost = (
        difference_between_final_assets_and_initial_money / initial_money
    )
